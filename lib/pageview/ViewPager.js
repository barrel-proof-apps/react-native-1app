...
var ViewPager = createReactClass({
  ...
  componentWillMount() {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: (e, gestureState) => {
        ...
        var offsetX2 = -dx / this.state.viewWidth + this.state.currentPage;
        this.state.scrollValue.setValue(offsetX);
        this.state.position.setValue(offsetX2);
      },
    });
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.dataSource) {
      ...
      if (!nextProps.isLoop) {
        ...
        this.state.position.setValue(constrainedPage);
      }
  ...
  movePage(step, gs, animate = true) {
    ...
    const postChange = () => {
      ...
      this.state.position.setValue(pageNumber);
      this.setState({
