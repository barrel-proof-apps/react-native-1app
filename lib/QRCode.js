...
function qr2bmp(data, colors, scale) {
  ...
  return `${header.toString('base64')}${outBuf.toString('base64')}`;
...
export function gerarQRcode(code,props={}){
  const {
    fgColor= '#000000',
    bgColor='#FFFFFF',
    size= 128,
  } = props;
  const QRData = qr(utf16to8(code)).modules;
  //
...
let QRCode=({value="QRCODE",size=200,...props})=>{
  return(
    <Image
      {...props}
      style={[{height:size,width:size},props.style]}
      source={{ uri: `data:image/bmp;base64,${gerarQRcode(value,{...props,size})}` }}
    />
  )
}

export default QRCode;
// QRCode.gerarQRcode=gerarQRcode;
