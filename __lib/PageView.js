...
export default class ApiPageView extends Component {
  ...
  constructor(props) {
    ...
    if (props.dataSource) {
      this.state.dataSource = this.state.dataSource.cloneWithPages(props.dataSource);
    }
    if (this.props.play) {
      this.ativo = true;
      this.loop();
  ...
  loop() {
    ...
    setTimeout(() => {
      if (this.props.play && this.ativo) {
        ...
        if (this.state.humano || this.getLength() == 0 || sleep - 1000 > dif) {
          //|| sleep - 1000 < dif
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (nextProps != this.props) {
      ...
      nextState.dataSource = this.state.dataSource.cloneWithPages(nextProps.dataSource);
      this.goToPage(0);
    }
  ...
  getIndicadores() {
    ...
    for (var i = 0; i < this.state.lista.length; i++) {
      itens.push(React.createElement(View, {
        key: i + "mark",
        style: {
          borderStyle: "solid",
          borderColor: "#000",
  ...
  render() {
    ...
    return React.createElement(
      ...
      React.createElement(ViewPager, _extends({
